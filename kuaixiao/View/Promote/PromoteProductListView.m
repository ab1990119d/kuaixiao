...
#import "PromoteProductListView.h"

@implementation PromoteProductListView
...
-(void)initTotalPrice{
    _totalPrice =0;
    for(PromoteProduct *detail in _orderDetails)
    {
        detail.totalPrice = detail.expectedSales *detail.origPrice;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    PromoteProduct *detail = [_orderDetails objectAtIndex:indexPath.row];
    ...
    numLabel.text = [[NSString alloc]initWithFormat:@"预期销量：%.0f %@ 价格：¥ %.2f",detail.expectedSales,detail.origMeasureName,detail.origPrice];
...
-(void)deleteDetail:(UIButton *)sender{
    ...
    PromoteProduct  *detail = [parms objectForKey:@"detail"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([self.delegate respondsToSelector:@selector(selectProduct:)]) {
        [self.delegate selectProduct:detail];
    }
}
